...
if (!isTouch && !reduceMotion){
    ...
    const shadow = document.getElementById('phoneShadow');  // grounding shadow on the floor
    ...
    stage.addEventListener('mousemove', (e) => {
        ...
        raf = requestAnimationFrame(() => {
            ...
            // grounding shadow drifts opposite the lean, like a real cast shadow
            if (shadow) shadow.style.transform =
                `translate(calc(-50% + ${(-cdx*20).toFixed(1)}px), calc(-50% + ${(-cdy*9).toFixed(1)}px)) scale(${(1 + Math.min(1,Math.hypot(cdx,cdy))*0.05).toFixed(3)})`;
        });
